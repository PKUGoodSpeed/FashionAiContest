...
class Trainer:
    ...
    def __init__(self, train_class_name=None, training_batch_size=100, existing_weight=None, test_percentage=0.02, learning_rate=0.0002, memory_safe=False):
        ...
        for class_index in range(self.num_classes):
            class_weight[class_index] = 1 /(class_count[class_index] / np.sum(class_count))
    ...
    def train(self, steps_per_epoch=10, epochs=100):
        ...
        self.model.fit(self.X_train, self.y_train, class_weight=self.class_weight, batch_size=self.training_batch_size, epochs=epochs, validation_data=(self.X_test, self.y_test), callbacks=[checkpoint, Validation(self.model, 5, self.num_classes, self.X_test, self.y_test)])
