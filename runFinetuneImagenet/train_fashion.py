...
if int(args.memory_safe) == 0:
	...
	trainer = TMS.Trainer(model_name=args.model_name, train_class_name=args.train_class_name, training_batch_size=int(args.training_batch_size), learning_rate=float(args.learning_rate), test_percentage=float(args.test_percentage), validation_every_X_batch=int(args.validation_every_X_batch), saving_frequency=float(args.saving_frequency))
